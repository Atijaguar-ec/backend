...
ADD COLUMN `order` INT DEFAULT 999 COMMENT 'Order for displaying facility types (lower = first)';
...
-- Set a high order value for any remaining facility types not explicitly ordered
UPDATE `FacilityType` SET `order` = 999 WHERE `order` IS NULL OR `order` = 0;
